...
def verify_mac(key, my_name, uid, cmd, download, mac):
    ...
    if my_name:
        message = 'myname={}&'.format(my_name)
    message += 'uid={}&lstcmd='.format(uid) + cmd + download_message
    payload = message  # 注意这里不再包含密钥

    # 使用 HMAC-SHA256 计算 MAC
    app.logger.debug('payload is [{}]'.format(payload))
    real_mac = hmac.new(
        key.encode('utf-8'),  # 将密钥编码为字节
        payload.encode('utf-8', 'surrogateescape'),  # 将消息编码为字节
        digestmod=hashlib.sha256  # 使用 SHA-256 作为哈希算法
    ).hexdigest()

    app.logger.debug('real mac is [{}]'.format(real_mac))

    # 比较计算得到的 MAC 与提供的 MAC
    return mac == real_mac
